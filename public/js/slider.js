...
const slider = {
  ...
  goToSlide: function (newPosition) {
    ...
    if (newPosition >= 0 && newPosition < sliderImagesElements.length) {
      ...
      document
        .querySelector(".slider__img--current")
        .classList.remove("slider__img--current");
    ...
    fetch("/", {
      ...
      .then((data) => {
        ...
        for (const imageName of data) {
          ...
          // Définit l'attribut 'src' de l'image avec le chemin de l'image locale
          newSliderImage.src = `/uploads/origamis/${imageName}`; // Chemin vers les images locales
          ...
          sliderContainer.appendChild(newSliderImage);
        }
